...
public class DepartmentResource {
    ...
    public Response createDepartment(@RequestBody Department department) throws JSONException, SQLException {
    ...
    @PUT
    @Path("")
    public Response updateDepartment(@RequestBody Department updatedDepartment) throws JSONException {
        try {
    ...
    @DELETE
    @Path("/{deptId}")
    public Response deleteDepartmentById(@PathParam("deptId") int deptId) throws JSONException {
        try {
            int rowsAffected = DepartmentService.deleteDepartmentById(deptId);
            if (rowsAffected > 0) {
                return Response.status(Response.Status.OK)
                        .entity("{\"message\": \"Department deleted successfully\"}")
                        .build();
            } else {
                JSONObject response = new JSONObject();
